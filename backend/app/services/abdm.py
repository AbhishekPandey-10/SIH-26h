...
async def request_otp(aadhaar_or_mobile: str) -> str:
    ...
    clean_id = aadhaar_or_mobile.strip()
    txn_id = f"txn_{uuid.uuid4().hex[:16]}"
    _ACTIVE_TRANSACTIONS[txn_id] = {
        "identifier": clean_id,
        "otp": "123456",  # Fixed sandbox OTP
        "created_at": datetime.now(UTC),
        "expires_at": datetime.now(UTC) + timedelta(minutes=10),
        "attempts": 0,
        "consumed": False,
    }
    logger.info(f"ABDM Sandbox: Generated transaction {txn_id} for identifier {clean_id}")
    return txn_id
...
async def verify_otp(txn_id: str, otp: str, abha_id: str | None = None) -> ABHASession:
    ...
    Enforces transaction existence, attempt limits, expiration, and replay prevention.
    ...
    txn = _ACTIVE_TRANSACTIONS.get(txn_id.strip())
    if not txn:
        raise ValueError("Invalid or unknown transaction ID. Please request a new OTP.")

    if txn.get("consumed", False):
        raise ValueError("OTP transaction has already been consumed and cannot be replayed.")

    now = datetime.now(UTC)
    if now > txn["expires_at"]:
        raise ValueError("OTP has expired. Please request a new OTP.")

    if txn.get("attempts", 0) >= 3:
        raise ValueError("Maximum OTP verification attempts exceeded. Transaction locked.")

    # Validate OTP code
    if txn["otp"] != otp.strip():
        txn["attempts"] = txn.get("attempts", 0) + 1
        remaining = 3 - txn["attempts"]
        if remaining <= 0:
            raise ValueError("Maximum OTP verification attempts exceeded. Transaction locked.")
        raise ValueError(f"Incorrect OTP entered. {remaining} attempt(s) remaining.")

    # Mark consumed immediately to block replay
    txn["consumed"] = True

    # Determine patient identity
    target_abha = abha_id or (txn["identifier"] if "@" in txn["identifier"] else "rajesh.kumar@abdm")
    ...
    logger.info(f"ABDM Sandbox: Session established for {patient.name} ({patient.abha_id}) via txn {txn_id}")
    return session


def clear_active_transactions() -> None:
    """Helper to clear in-memory transactions during testing."""
