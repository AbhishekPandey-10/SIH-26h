...
async def interview_websocket(websocket: WebSocket, session_id: str | None = None):
    ...
    try:
        # 1. Inject Smart Recall context across patient sessions
        extracted_context = await get_patient_extracted_context(active_session_id)

        # 2. Send the first question upon connection
